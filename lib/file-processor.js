...
const getMostPopularWord = async (content) => {
  ...
  const mostPopularOccurencies = Object.values(countedWords).reduce((word, curr) =>
    (word > curr) ? word : curr
  )

  // get word(s) with most occurencies
  return Object.entries(countedWords).filter((x) => x[1] === mostPopularOccurencies).map(x => x[0])
}
